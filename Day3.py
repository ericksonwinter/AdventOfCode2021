...
powerConsumption = [i.strip() for i in dataset.split("\n") if i]

# read set as long string to be able to split in to list of ints below
def Part1():

    matrix = [0] * len(powerConsumption[0])
    gammaRate = matrix
    epsilonRate = matrix    
    for r in powerConsumption:
        for i in range(len(r)-1):
            matrix[i] += (-1, 1)[int(r[i])]

            # debug
            logging.debug(f'matrix: {matrix}')

    gammaRate = int(''.join([str((0, 1)[i >= 0]) for i in matrix]), 2)
    epsilonRate = int(''.join([str((0, 1)[i < 0]) for i in matrix]), 2)
