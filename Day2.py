...
def Part1():
    ...
    for r in positionReadings:
        if list(r)[0] == 'forward': position.x += list(r.values())[0]
        if list(r)[0] == 'down': position.y += list(r.values())[0]
...
def Part2():
    ...
    position = dotDict({'x': 0, 'y': 0, 'aim': 0})
    for r in positionReadings:
        if list(r)[0] == 'forward': 
            position.x += list(r.values())[0]
            position.y += list(r.values())[0] * position.aim
        if list(r)[0] == 'down': position.aim += list(r.values())[0]
